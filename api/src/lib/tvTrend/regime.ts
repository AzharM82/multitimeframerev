...
export type Action = "SPY CALLS" | "SPY PUTS" | "CLOSE" | "NONE";

export interface Recommendation {
  action: Action;
  /** One line explaining why, always populated — including for NONE. */
  why: string;
  aligned: boolean;
}

/**
 * Combine streak + regime into a recommendation.
 *
 * Operator's rules, 2026-08-09:
 *   - ALIGNED ONLY. green streak needs a bullish regime, red needs bearish.
 *     A conflict or a neutral regime alerts but recommends nothing.
 *   - Gate decision NO suppresses (stand down); CAUTION recommends with a flag.
 *   - trend_end always means close, regardless of regime — you cannot be gated
 *     out of an exit.
 */
export function recommend(
  ...
  if (event === "trend_end") {
    const side = trend === "green" ? "SPY CALLS" : "SPY PUTS";
    return { action: "CLOSE", why: `${trend.toUpperCase()} streak over — close ${side} if held`, aligned: true };
  }

  if (!regime) {
    return { action: "NONE", why: "no regime on record yet — cannot qualify this streak", aligned: false };
  }
  if (regimeStale) {
    return {
      action: "NONE",
      why: `regime is ${Math.round(ageMinutes(regime.capturedAt))}m stale (${regime.label}) — not trusted for today`,
      aligned: false,
    };
  }
  if (regime.decision === "NO") {
    return { action: "NONE", why: `Gate says NO today (quality ${regime.qualityScore}) — stand down`, aligned: false };
  }

  const want: RegimeDirection = trend === "green" ? "bullish" : "bearish";
  if (regime.direction !== want) {
    const how = regime.direction === "neutral" ? `regime is neutral (${regime.label})` : `counter-trend to a ${regime.label} regime`;
    return { action: "NONE", why: `${trend.toUpperCase()} streak but ${how} — no recommendation`, aligned: false };
  }

  const action: Action = trend === "green" ? "SPY CALLS" : "SPY PUTS";
  const caution = regime.decision === "CAUTION" ? " [Gate: CAUTION]" : "";
  return { action, why: `${trend.toUpperCase()} streak aligned with ${regime.label}${caution}`, aligned: true };
}
