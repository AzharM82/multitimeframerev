...
const FALLBACK: Params = { waitMin: 10, emaLen: 9, targetPct: 20, stopPct: 9, trail: [{ atPct: 15, stopPct: 5 }], commissionRt: 0, accountUsd: 2500 };
    ...
    <div className="space-y-3">
      ...
      <Card title={`4 · Exit: +${p.targetPct}% target, −${p.stopPct}% stop that rises with the trade, else the close`}>
        ...
        <ul className="text-[11px] text-text-secondary space-y-1 list-disc pl-4">
          ...
          <li><b className="text-text-primary">The stop rises once</b> (added 2026-09-10). {trail.map((t, i) => <span key={t.atPct}>{i ? "; " : ""}once a bar&apos;s high has been +{t.atPct}% above the entry, the stop moves to entry × {(1 + t.stopPct / 100).toFixed(2)} (+{t.stopPct}%)</span>)}. The stop is only ever raised, and the raise applies from the <em>next</em> bar, because inside one bar we cannot tell whether the high or the low printed first. An exit on the raised stop is shown as &ldquo;raised stop&rdquo;: a locked-in gain instead of a round trip to the stop.</li>
...
const GLOSSARY: [string, string][] = [
  ...
  ["Why", "Which exit fired: target, raised stop (+5% locked in after the trade had been up 15%), stop, or close."],
