...
- **The Unusual Options sweep runs in `mtfrev-cron`, not in the API** (rebuilt here 2026-09-13; the separate github.com/AzharM82/UnusualOptions repo is retired — its last scan was 6 July with a universe of 2 and no open interest). SWA cuts a managed API request off at **45 seconds** and sweeping 127 symbols against Tradier takes ~4.3 minutes, so `tools/cron-functions/src/uoa/` does the work and writes `uoa-signals/{date}.json`, `latest.json` and `baselines.json`; the portal keeps only the read proxy `GET /api/uoa-signals`. The cron app therefore needs `TRADIER_TOKEN` and `AZURE_STORAGE_CONNECTION_STRING` of its own, and `host.json` sets `functionTimeout: 00:10:00`. Run `node test/uoa-detection-test.mjs` (60 assertions, no network) before changing a threshold, and `node test/uoa-run-once.mjs <YYYY-MM-DD> [--dry]` to sweep by hand. The headline test is volume ÷ prior-settlement open interest; the 20-day volume ratio is secondary and stays disabled per contract until `baselines.json` holds 10 sessions for it. A run that reaches under 60% of the universe refuses to publish rather than overwriting a good scan with an empty one.
