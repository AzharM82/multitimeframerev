...
- **Unusual Options is LIVE intraday, with the end-of-day sweep as the record.** Both run in `mtfrev-cron`, never in the API — SWA cuts a managed request off at **45 seconds**. Three timers in `tools/cron-functions/src/uoa/`:
  - `uoaWatchsetCron` **08:40 ET** — walks the chains (3-45 DTE, nearest 4 expiries, within 20% of spot, OI >= 25) and writes `watchset-{date}.json`: ~14.7k contracts, ~5.4 min. Pre-open on purpose, while StockAgentHub's executor is idle and the whole 120/min Tradier budget is ours (`setPace`).
  - `uoaLiveCron` **every 2 min, self-gated 09:30-16:00 ET** — one form **POST** `/markets/quotes` per ~900 contracts (a GET over ~300 symbols returns 414); the whole watch set refreshes in 17 calls / 12 s, so polling costs ~8.5 calls a minute. Diffs against `live-state-{date}.json`, writes `live-{date}.json` + `live.json`. **The first poll of a day reports nothing on purpose** (`warming`) — with no earlier reading every contract's whole morning is one burst. A poll returning under 80% of the watch set is skipped, not published.
  - `uoaScanCron` **17:05 ET** — the end-of-day sweep (127 symbols, ~4.3 min) writing `{date}.json`, `latest.json`, `baselines.json`.

  **Phone alerts (WhatsApp + Pushover) fire from `uoaLiveCron`.** Same mechanism as everywhere else — the `whatsapp-alerts` storage queue that the local whatsapp-web.js sidecar drains, plus Pushover — ported into the cron app as `src/uoa/notify.js` because this app has no build step to share the API's TypeScript. **The alert bar is far above the tab's**: $1m committed in one window OR the window alone trading 2x all outstanding interest, then grouped to **one line per underlying and one message per poll** (the AVWAP rule: never one message per ticker), a 30-minute cooldown per name unless it returns 3x larger, and a hard cap of 15 messages a session. Without that, a uniform two-minute slice already yields four bursts and 195 polls a day is an unreadable stream. `UOA_ALERTS=off` disables them; `UOA_ALERT_TEST=1` sends one labelled test message and does nothing else (unset it straight after). By-hand polls are **dry-run by default** — `test/uoa-live-once.mjs poll` never sends unless you add `--send`. Needs `PUSHOVER_APP_TOKEN`, `PUSHOVER_USER_KEY`, `WHATSAPP_RECEIVER` on the cron app. Test with `node test/uoa-alerts-test.mjs` (38 assertions, no network).

  **The live rule is the increment, not the total**: contracts traded since the previous poll, firing at >=15% of prior open interest OR >=$500k committed, over floors of 250 lots / $50k / $0.10 / OI 25. The EOD rule (cumulative volume over OI) is deliberately different — cumulative volume only grows, so it fires late and cannot say *when* anything happened. **Open interest does not move intraday** (OCC recomputes it overnight), so `prior_oi` is yesterday's settlement all session; never present it as spiking. **Buy versus sell cannot be known** — `side` is only where the last print sat in the spread and must stay labelled a lean. Tests: `node test/uoa-burst-test.mjs` (37) and `node test/uoa-detection-test.mjs` (60), no network. By hand: `node test/uoa-live-once.mjs build|poll [date]` and `node test/uoa-run-once.mjs <date> [--dry]`. The cron app needs `TRADIER_TOKEN` + `AZURE_STORAGE_CONNECTION_STRING`; `host.json` sets `functionTimeout: 00:10:00`. A sweep reaching under 60% of the universe refuses to publish. The old github.com/AzharM82/UnusualOptions repo is retired and its Actions cron disabled.
